...
var totalPointsFormula = function (row) {
  ...
  // Deal with last 8 columns in last separate loop
...
var inputMainBodyFormulas = function (ws, row) {
  ...
  ws.cell(row, 17).formula('=HK' + row + '*HK4').style(this.styles.nonEditableCaseStyle) // Parom Points
  ws.cell(row, 18).formula('=HL' + row + '*HL4').style(this.styles.nonEditableCaseStyle) // ARMS Comm Points
  ws.cell(row, 19).formula('=HM' + row + '*HM4').style(this.styles.nonEditableCaseStyle) // ARMS Licence Points
  ws.cell(row, 20).formula(totalPointsFormula(row)).style(this.styles.nonEditableCaseStyle) // Total Caseload Points
  ...
  ws.cell(row, 23).formula('=V' + row + '-U' + row).style(this.styles.roundedStyle) // Remaining Points
...
var inputBottomTotals = function (ws, row) {
  ...
  ws.cell(row, 24).formula('=IFERROR(U' + row + '/V' + row + ',0)').style(this.styles.averagePercentageStyle)
  ...
  for (secondLetter = 0; secondLetter < 13; secondLetter++) {
    char1 = 'H'
    char2 = String.fromCharCode(secondLetter + 65)
    ws.cell(row, columnNo).formula('=SUM(' + char1 + char2 + 5 + ':' + char1 + char2 + dataEndRow + ')').style(this.styles.sumStyle)
    columnNo = columnNo + 1
