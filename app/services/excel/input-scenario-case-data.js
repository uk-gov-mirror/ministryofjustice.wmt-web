...
var totalPointsFormula = function (row) {
  ...
  var formula = '=SUM(V4*V' + row + ',W4*W' + row + ',X4*X' + row + ',Y4*Y' + row + ',Z4*Z' + row
  var firstLetter
  var secondLetter
  var char1
  var char2
...
var inputFormulas = function (ws, row) {
  ...
  ws.cell(row, 15).formula('=HI' + row + '*HI4') // ARMS Comm Points
  ws.cell(row, 16).formula('=HJ' + row + '*HJ4') // ARMS Licence Points
  ws.cell(row, 17).formula(totalPointsFormula(row)) // Total Caseload Points
  ws.cell(row, 18).formula('=SUM(H' + row + ',J' + row + ',L' + row + ':Q' + row + ')') // Overall Total Points
  ws.cell(row, 19).formula('=IFERROR(ROUNDDOWN(((D' + row + ' * (E' + row + '/F' + row + '))*((E' + row + '-G' + row + ')/E' + row + ')),0),0)').style(this.roundedStyle) // Available Points
  ws.cell(row, 20).formula('=S' + row + '-R' + row).style(this.roundedStyle) // Remaining Points
  ws.cell(row, 21).formula('=IFERROR(R' + row + '/S' + row + ',0)').style(this.percentageStyle) // Current % Capacity
}
...
var inputTotals = function (ws, row) {
  ...
  ws.cell(row, 19).formula('=SUM(S' + 5 + ':S' + dataEndRow + ')').style(this.roundedSumStyle)
  ws.cell(row, 20).formula('=SUM(T' + 5 + ':T' + dataEndRow + ')').style(this.roundedSumStyle)
  ws.cell(row, 21).formula('=IFERROR(R' + row + '/S' + row + ',0)').style(this.averagePercentageStyle)
