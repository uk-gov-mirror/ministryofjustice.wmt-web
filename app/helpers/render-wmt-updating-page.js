...
module.exports = function (res, userRole, authorisation, subNav) {
  return getLatestProcessImportTask().then(function (result) {
    var ETAMinutes = moment(result.date_created).format('mm')
    var minutesToAdd = minutesToRoundTo - (parseInt(ETAMinutes) % minutesToRoundTo)
    var ETA = moment(result.date_created).add(1, 'hours').add(minutesToAdd, 'minutes').format('h:mm a')
    var ETAPassed = moment().isAfter(moment(result.date_created).add(1, 'hours').add(minutesToAdd, 'minutes'))
    if (ETAPassed) {
