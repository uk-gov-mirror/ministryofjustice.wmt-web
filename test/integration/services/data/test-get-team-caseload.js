  ...
  it('should retrieve current caseload overview for a workload owner on a team', function () {
    return getTeamCaseload(inserts.filter((item) => item.table === 'team')[1].id, 'team')
      .then(function (results) {
        ...
        expect(results[0].untiered).to.eql(30)
        expect(results[0].d2).to.eql(30)
        expect(results[0].d1).to.eql(30)
        expect(results[0].c2).to.eql(30)
        expect(results[0].c1).to.eql(30)
        expect(results[0].b2).to.eql(30)
        expect(results[0].b1).to.eql(30)
        expect(results[0].a).to.eql(30)
